#!/usr/bin/env python3
"""茶叶百科v6.0 补全缺失的render函数 - V2 (target correct container IDs)"""
...
print(f"Stub block: lines {stub_start+1}-{stub_end+1}")
...
# Container IDs are: glossaryGrid, newsList, seasonContent, standardContent
new_stub_block = '''// ============================================================
// 术语词典
// ============================================================
function renderTermPage() {
  var el = document.getElementById('glossaryGrid');
  if (!el) { var altEl = document.getElementById('termPageContent'); if (altEl) el = altEl; }
  if (!el) return;
  var categories = [
...
function renderNewsPage() {
  ...
  if (!el) { var altEl = document.getElementById('newsPageContent'); if (altEl) el = altEl; }
  if (!el) return;
...
function renderSeasonPage() {
  var el = document.getElementById('seasonContent');
  if (!el) { var altEl = document.getElementById('seasonPageContent'); if (altEl) el = altEl; }
  if (!el) return;
  var seasons = [
    {
...
function renderStandardPage() {
  var el = document.getElementById('standardContent');
  if (!el) { var altEl = document.getElementById('standardPageContent'); if (altEl) el = altEl; }
  if (!el) return;
...
# Verify no duplicate page divs were added
import re
term_page_count = len(re.findall(r'id="termPage"', verify))
news_page_count = len(re.findall(r'id="newsPage"', verify))
season_page_count = len(re.findall(r'id="seasonPage"', verify))
standard_page_count = len(re.findall(r'id="standardPage"', verify))

print(f"  termPage div count: {term_page_count} (should be 1)")
print(f"  newsPage div count: {news_page_count} (should be 1)")
print(f"  seasonPage div count: {season_page_count} (should be 1)")
print(f"  standardPage div count: {standard_page_count} (should be 1)")
